...
import ext.mods.gameserver.helpers.ItemHelper;
...
public class StartingItemsManager {
    ...
    private void giveItemToPlayer(Player player, int itemId, int quantity) {
        try {
            ItemInstance item = player.getInventory().addItem(itemId, quantity);
            
            if (item != null) {
                LOGGER.debug("Item dado: " + item.getName() + " (ID: " + itemId + ") x" + quantity + " para " + player.getName());
                player.sendMessage("Recebido: " + ItemHelper.getFormattedItemName(player, item) + " x" + quantity);
            } else {
